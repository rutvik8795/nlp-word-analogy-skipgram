...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  label_index = 0;
  batch_index = 0;
  while data_index < len(data)-skip_window:
    left_index = data_index - 1
    right_index = data_index + 1
    while left_index>=data_index-skip_window and right_index<=data_index+skip_window and len(batch)<=batch_size:
      batch[batch_index] = data[data_index]
      batch_index = batch_index+1
      batch[batch_index] = data[data_index]
      batch_index = batch_index+1
      labels[label_index,0] = data[left_index]
      label_index = label_index+1
      labels[label_index,0] = data[right_index]
      label_index = label_index+1
...
def build_model(sess, graph, loss_model):
  ...
  with graph.as_default():
    ...
    optimizer = tf.train.GradientDescentOptimizer(25.0).minimize(loss, global_step=global_step)
...
if __name__ == '__main__':
  ...
  # batch, labels = generate_batch(data, batch_size=64, num_skips=2, skip_window=1)
  # print("start to print batch and labels")
  # for i in range(64):
